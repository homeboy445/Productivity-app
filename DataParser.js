var ret_obj=(data)=>{
    var obj1=data.split('|');
    var date_str=obj1[0],task_str=obj1[1];
    var date='',id='',task='',priority='',turn=1,str='';
    ...
    for(var j=0;j<task_str.length;j++)
    {
        if(task_str[j]=='+')
        {
            ...
            {
                case 1:id=str; break;
                case 2:task=str; break;
                case 3:priority=str; break;
            }turn++; str='';
        }
        else
        {
...
const CalcSecond=(item)=>{
    ...
    var time_tk=`${hrs<10?"0"+hrs:hrs}:${min<10?"0"+min:min}:${sec<10?"0"+sec:sec}`;
    return [{
        date:item[0].date,
        task:item[0].task,
        time_taken:time_tk
...
const GetCompleteData=()=>{
    var data=fs.readFileSync(path.join(__dirname+"/Completed.txt")).toString().split('\n');
    var comp=[];
    data.map(item=>{
        if(item.trim())
