...
const {DeleteTask}=require(path.join(__dirname+"/DeleteTask.js"));
var interval_start=0,interval_resume=0,flag=false,current_task=null;
var resume_tracker=0,Theme_Manager;
var Done=[],counter=0;
...
const Resume_Timer=()=>{
        ...
        resume.addEventListener('click',()=>{
            if(resume_tracker==0)
            {
                interval_resume=setInterval(()=>{timerIncrementer("Resume")},1000);
                resume_tracker++;
            }
            reset.disabled=true;
            start.disabled=true;
...
start.addEventListener('click',async ()=>{
    ...
    var color=(task)=>{
        ...
        }return found!=-1?found=="high"?"red":found=="mid"?"blue":"yellow":"white";
    }
    cur.style.color=color(cur_task.task);
    cur.innerText=cur_task.task;
    interval_start=setInterval(()=>{timerIncrementer("Start")},1000);
    start.disabled=true; reset.disabled=true; flag=false;
});
...
reset.addEventListener('click',async ()=>{
    if(flag)
    {
        ...
        if(proceed.status)
        {
            ...
            var obj=`[${new Date().toLocaleDateString()}]|{${completed_task}=(${hrs.innerText}:${min.innerText}:${sec.innerText})}\n`;
            fs.appendFile(path.join(__dirname,'Completed.txt'),obj,(e)=>{
                return;
            })
            if(proceed.verdict!='Next Project')
...
const AppendTask=(obj)=>{
    ...
    var color=obj.priority=="mid"?"rgb(49, 214, 255)"
    :obj.priority=="high"?"red":"yellow";
